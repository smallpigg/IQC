...
df["IQC文件名称"] = df["物资名称"]
for i in range(0, len(df)):
    str1 = df.loc[i, 'IQC文件编号'] + '_' + df.loc[i, '物资名称'] + '进货检验作业指导书.docx'
    df.loc[i, 'IQC文件名称'] = str1

# 增加IQC文件记录文件名称
df["IQC记录文件名称"] = df["物资名称"]
for i in range(0, len(df)):
    str1 = 'TB-' + df.loc[i, 'IQC文件编号'] + '_' + df.loc[i, '物资名称'] + '进货检验记录.docx'
...
for record in df.to_dict(orient="records"):
    ...
    for i in range(1, 10):
        if record['项目'+str(i)] == "尺寸1":
            ...
            string_set = set(['尺寸', '材料', '材质', '产品包装', '单证资料', '规格型号', '合格证明', '认证资料', '产品描述'])
            row = table1.add_row()
            ...
            if all([word in string_set for word in list_string]):
                run = table1.cell(len(table1.rows) - 1, 3).paragraphs[0].add_run("")
            else:
                run = table1.cell(len(table1.rows) - 1, 3).paragraphs[0].add_run("")
            run.font.name = u'宋体'
